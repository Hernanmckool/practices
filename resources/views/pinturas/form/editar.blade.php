{!!Form::model($pint,['route'=>['pinturas.update',$pint->id], 'method'=>'PUT', 'files'=> true])!!}
<div class="box box-info">
    <div class="box-header with-border">
      <h3 class="box-title">Editar Pinturas</h3>
    </div>
    <div class="row">
        <div class="col-lg-6">
        ...
        <div class="input-group">
          <span class="input-group-addon"><i class="fa fa-pencil"></i></span>
            ...
            <option value="" disabled="true" selected="true">Seleccione una Categoria</option>
              @foreach($cats as $cat)
              <option <?php if($cat->id == $pint->id_categoria) echo 'selected="selected"';?> value="{{$cat->id}}">{{$cat->categoria}}</option>
              @endforeach
            </select>
        </div>
        </div><!-- /.col-lg-6 -->
